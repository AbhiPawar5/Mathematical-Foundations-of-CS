...
def calculate_population_mean():
    ...
    df = typecasted[3]-1
    mean = typecasted[2]
    #As confidence interval is given,it's 2 tail test and 
    # p = 0.975 i.e 2.5 % on both sides
    confidence_interval = round(confidence_interval,3)
    tc = stats.t.ppf(confidence_interval,df) 
    
    #print("Tc for Confidence Interval is : ",abs(tc))
...
root = Tk()
root.title("MFCS Confidence Interval")
Label(root,text="Simple Confidence Interval Application by Abhishek Pawar(009)").grid(row=0,column=0,sticky=W,pady=10)

#GUI CODE For simple Tc
Label(root,text="Enter Conf. Interval and N : ").grid(row=1,column=0,sticky=W)
textTc = StringVar()
Entry(root,textvariable=textTc).grid(row=1,column=1,sticky=W)
Button(root,text="Calculate Simple Tc",command=calculate_tc).grid(row=2,column=0,sticky=W)


#GUI CODE for Calculate error here.
Label(root,text="Enter Conf. Interval , s , mean and n : ").grid(row=3,column=0,sticky=W)
textErr = StringVar()
Entry(root,textvariable=textErr).grid(row=3,column=1,sticky=W)
Button(root,text="Calculate Population Mean",command=calculate_population_mean).grid(row=4,column=0,sticky=W)


#GUI CODE for Calculate population proportion here.
Label(root,text="Enter Conf. Interval , x and n : ").grid(row=5,column=0,sticky=W)
textPop = StringVar()
Entry(root,textvariable=textPop).grid(row=5,column=1,sticky=W)
Button(root,text="Calculate Population Proportion",command=calculate_pop_proportion).grid(row=6,column=0,sticky=W)


#GUI CODE for Calculate population variance and sd here.
Label(root,text="Enter Conf. Interval , sd and n :").grid(row=7,column=0,sticky=W)
textVarSd = StringVar()
Entry(root,textvariable=textVarSd).grid(row=7,column=1,sticky=W)
Button(root,text="Calculate Population Variance and SD",command=calculate_pop_variance_sd).grid(row=8,column=0,sticky=W)
